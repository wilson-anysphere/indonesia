...
- `crates/nova-workspace/tests/suite/real_projects.rs` (run via `cargo test --locked -p nova-workspace --test workspace_events -- --ignored`)
- `crates/nova-cli/tests/real_projects.rs` (run via `cargo test --locked -p nova-cli --test real_projects -- --ignored`)
