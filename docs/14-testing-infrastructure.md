...
**Where:**
 ...
 - Tests: `crates/nova-types/tests/suite/javac_differential.rs` (included by `crates/nova-types/tests/integration.rs`)
...
| `.github/workflows/javac.yml` | in repo | Run `#[ignore]` `javac` differential tests in an environment with a JDK | `cargo test -p nova-types --test integration javac_differential -- --ignored` |
