...
- makes it feasible to reuse the semantic layer (currently split across multiple crates) and `nova-ide` in other tools (CLI, batch analysis).
...
- **Commands**:
  - `cargo run --locked -p nova-devtools -- check-deps` — validate workspace dependency edges against layer policy.
  - `cargo run --locked -p nova-devtools -- check-layers` — ensure `crate-layers.toml` stays in sync with workspace members.
  - `cargo run --locked -p nova-devtools -- check-architecture-map --strict` — ensure `docs/architecture-map.md` stays in sync with workspace crates.
  - `cargo run --locked -p nova-devtools -- check-protocol-extensions` — ensure `docs/protocol-extensions.md` stays in sync with `nova-lsp` + editor client usage.
