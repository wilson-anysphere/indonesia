...
- **Gradle build integration (snapshot handoff `.nova/queries/gradle.json`):**
  [`gradle-build-integration.md`](gradle-build-integration.md)
...
- **Distributed mode (docs/16-distributed-mode.md):**
  ...
  - Current scope is intentionally narrow/experimental:
    ...
    - See `crates/nova-lsp/tests/suite/stdio_distributed_workspace_symbol.rs` (compiled into the `nova-lsp` `--test stdio` harness at `crates/nova-lsp/tests/harness/stdio.rs`) for an end-to-end stdio integration test covering the distributed `workspace/symbol` flow.
- **Protocol extensions:**
  ...
  - Safe-mode is a shipped resilience feature for Nova-specific endpoints. Panics and (most) watchdog timeouts in watchdog-wrapped `nova/*` handlers can temporarily enable safe-mode.
    - While safe-mode is enabled, `nova_lsp::hardening::guard_method` rejects most `nova/*` requests; only `nova/bugReport`, `nova/metrics`, `nova/resetMetrics`, and `nova/safeModeStatus` are always allowed (see `guard_method` in `crates/nova-lsp/src/hardening.rs`). Standard LSP methods continue to work.
    - The binary surfaces state changes via `nova/safeModeChanged` (see `flush_safe_mode_notifications` in `crates/nova-lsp/src/main.rs`) and supports polling via `nova/safeModeStatus`.
