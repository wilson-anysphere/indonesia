...
- **Distributed mode (docs/16-distributed-mode.md):**
  ...
  - Current scope is intentionally narrow/experimental:
    ...
    - See `crates/nova-lsp/tests/suite/stdio_distributed_workspace_symbol.rs` (compiled into the `nova-lsp` `--test tests` harness at `crates/nova-lsp/tests/tests.rs`) for an end-to-end stdio integration test covering the distributed `workspace/symbol` flow.
- **Protocol extensions:**
