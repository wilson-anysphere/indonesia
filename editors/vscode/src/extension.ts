...
export async function activate(context: vscode.ExtensionContext) {
  ...
  async function installOrUpdateDebugAdapter(): Promise<void> {
    ...
    try {
      ...
      const check = await checkBinaryVersion(installed.path);
      if (!check.ok || !check.version) {
        const suffix = check.version
          ? `found v${check.version}, expected v${extensionVersion}`
          : check.error
        ...
        if (choice === 'Make Executable') {
          const updated = await makeExecutable(installed.path);
          if (updated) {
            const rechecked = await checkBinaryVersion(installed.path);
            if (rechecked.ok && rechecked.version) {
              return;
            }
          }
  ...
  async function useLocalDebugAdapterBinary(): Promise<void> {
    ...
    if (!check.ok || !check.version) {
      ...
      actions.push('Cancel');
      const choice = await vscode.window.showErrorMessage(`Nova: selected nova-dap is not usable (${suffix}): ${dapPath}`, ...actions);
      if (choice === 'Make Executable') {
  ...
  async function showDebugAdapterVersion(): Promise<void> {
    ...
    if (!resolved) {
      const message = settings.path ? `Nova: nova.dap.path points to a missing file: ${settings.path}` : 'Nova: nova-dap is not installed.';
      const action = await vscode.window.showErrorMessage(
        message,
