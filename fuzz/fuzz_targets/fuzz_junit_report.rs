    ...
    RUNNER.get_or_init(|| {
        ...
        std::thread::spawn(move || {
            for input in input_rx {
                ...
                let _ = nova_testing::report::parse_junit_report_str(&input);
...
fuzz_target!(|data: &[u8]| {
    ...
    runner
        .input_tx
        .send(text.to_owned())
        .expect("fuzz_junit_report worker thread exited");

    match runner
        .output_rx
        .lock()
        .expect("fuzz_junit_report worker receiver poisoned")
        .recv_timeout(TIMEOUT)
    {
        Ok(()) => {}
        Err(mpsc::RecvTimeoutError::Timeout) => panic!("fuzz_junit_report fuzz target timed out"),
        Err(mpsc::RecvTimeoutError::Disconnected) => {
            panic!("fuzz_junit_report worker thread panicked")
        }
    }
});
