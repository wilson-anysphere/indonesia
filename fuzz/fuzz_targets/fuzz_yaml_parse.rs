    ...
    RUNNER.get_or_init(|| {
        ...
        std::thread::spawn(move || {
            for input in input_rx {
                ...
                let parsed = nova_yaml::parse(&input);
...
fuzz_target!(|data: &[u8]| {
    ...
    runner
        .input_tx
        .send(text.to_owned())
        .expect("fuzz_yaml_parse worker thread exited");

    match runner
        .output_rx
        .lock()
        .expect("fuzz_yaml_parse worker receiver poisoned")
        .recv_timeout(TIMEOUT)
    {
        Ok(()) => {}
        Err(mpsc::RecvTimeoutError::Timeout) => panic!("fuzz_yaml_parse fuzz target timed out"),
        Err(mpsc::RecvTimeoutError::Disconnected) => panic!("fuzz_yaml_parse worker panicked"),
    }
});
