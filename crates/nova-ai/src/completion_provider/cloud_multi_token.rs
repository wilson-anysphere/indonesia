...
fn redact_file_paths(text: &str) -> String {
    ...
    static WINDOWS_PATH_RE: once_cell::sync::Lazy<regex::Regex> = once_cell::sync::Lazy::new(|| {
...
fn parse_completions(
    ...
    for item in items.take(max_items) {
        ...
        let additional_edits = parse_additional_edits(obj.get("additional_edits"), max_additional_edits);
...
fn parse_additional_edits(value: Option<&Value>, max_additional_edits: usize) -> Vec<AdditionalEdit> {
    ...
    for item in items.iter().take(max_additional_edits) {
        ...
        out.push(AdditionalEdit::AddImport { path: path.to_string() });
    }
...
fn default_label(insert_text: &str) -> String {
    let mut label = insert_text.lines().next().unwrap_or(insert_text).trim().to_string();
    if label.is_empty() {
