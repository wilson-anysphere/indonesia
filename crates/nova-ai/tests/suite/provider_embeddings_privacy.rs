...
#[test]
fn provider_embeddings_http_loopback_url_is_allowed_in_local_only_mode() {
    let server = MockServer::start();
    let mock = server.mock(|when, then| {
        when.method(POST)
            .path("/v1/embeddings")
...
#[tokio::test(flavor = "current_thread")]
async fn provider_embeddings_client_http_loopback_url_is_allowed_in_local_only_mode() {
    let server = MockServer::start();
    let mock = server.mock(|when, then| {
        when.method(POST).path("/v1/embeddings");
        then.status(200).json_body(json!({
