...
#[test]
fn related_code_query_skips_hex_emitted_unicode_escape_path_only_selections_short_paths() {
    struct PanicSearch<'a> {
        focal_code: &'a str,
    }
    ...
    impl SemanticSearch for PanicSearch<'_> {
        fn search(&self, query: &str) -> Vec<SearchResult> {
            panic!(
                "search should not be called for path-only selections where a hex escape emits a unicode-escape prefix; focal_code={}; got query={query}",
                self.focal_code
            );
        }
    }

    // `x{75}` decodes to `u`, yielding `u002F...` and then `/...` across multiple decode passes.
    for focal_code in ["x{75}002Fsrc", "x{75}002Fsrcx{75}002Fmain"] {
        let search = PanicSearch { focal_code };
        let req = base_request(focal_code).with_related_code_from_focal(&search, 3);
        assert!(
            req.related_code.is_empty(),
            "expected no related code for hex-emitted unicode-escape prefix in short path-only focal code"
        );
    }
}
...
#[test]
fn related_code_query_skips_unicode_escape_percent_marker_with_entity_digits_short_paths() {
    struct PanicSearch;

    impl SemanticSearch for PanicSearch {
        fn search(&self, query: &str) -> Vec<SearchResult> {
            panic!(
                "search should not be called for unicode percent markers with entity-encoded digits in short path selections; got query={query}"
            );
        }
    }
    ...
    let search = PanicSearch;
    for focal_code in [
        // HTML numeric entities that decode to the digits of `u0025` (percent sign).
        "u&#x30;&#x30;&#x32;&#x35;u0032u0046src", // %2Fsrc
        "u&#x30;&#x30;&#x32;&#x35;u0035u0043src", // %5Csrc
    ] {
        let req = base_request(focal_code).with_related_code_from_focal(&search, 3);
        assert!(
            req.related_code.is_empty(),
            "expected no related code for short path-only focal code with digit-entity unicode percent markers"
        );
    }
}
...
#[test]
fn related_code_query_skips_internal_obfuscated_percent_marker_short_paths() {
    struct PanicSearch;

    impl SemanticSearch for PanicSearch {
        fn search(&self, query: &str) -> Vec<SearchResult> {
            panic!(
                "search should not be called for internal obfuscated percent markers in short path selections; got query={query}"
            );
        }
    }
    ...
    let search = PanicSearch;
    for focal_code in [
        // `x25` where the `25` digits are unicode escapes, embedded inside an identifier token.
        "homexu0032u0035u0032u0046user", // home%2Fuser
        "homexu0032u0035u0035u0043user", // home%5Cuser
        // `u0025` where the `0025` digits are hex escapes, embedded inside an identifier token.
        "homeux30x30x32x35u0032u0046user", // home%2Fuser
        "homeux30x30x32x35u0035u0043user", // home%5Cuser
    ] {
        let req = base_request(focal_code).with_related_code_from_focal(&search, 3);
        assert!(
            req.related_code.is_empty(),
            "expected no related code for short path-only focal code with internal obfuscated percent markers"
        );
    }
}
...
#[test]
fn related_code_query_skips_escaped_percent_encoded_path_only_selections_with_hex_escaped_percent_markers_and_braced_hex_digit_encodings(
) {
    struct PanicSearch<'a> {
        sep: &'a str,
    }
    ...
    impl SemanticSearch for PanicSearch<'_> {
        fn search(&self, query: &str) -> Vec<SearchResult> {
            panic!(
                "search should not be called for escaped percent-encoded path selections with hex-escaped percent markers and braced hex digit encodings (sep={}); got query={query}",
                self.sep
            );
        }
    }
    ...
    for sep in [
        // Percent marker encoded as `x25` with braced unicode digit escapes.
        "x25u{0032}u{0046}", // %2F
        "x25u{0035}u{0043}", // %5C
        // Mixed braced/unbraced digit escapes after a hex-escaped percent marker.
        "x25u0032u{0046}", // %2F
        "x25u0035u{0043}", // %5C
        "x25u{0032}u0046", // %2F
        "x25u{0035}u0043", // %5C
        ...
        assert!(
            req.related_code.is_empty(),
            "expected no related code for escaped percent-encoded path-only focal code with hex-escaped percent markers and braced hex digit encodings"
        );
    }
}
...
fn related_code_query_skips_html_entity_percent_encoded_path_only_selections_without_semicolons_with_escaped_hex_digits(
    ...
    for sep in [
        ...
        "u0026&#35;37u0032u0046",      // %2F
        "u0026&#x23;37u0032u0046",     // %2F
        // Number sign entity building a hex numeric percent entity (`#x25`) where the base marker
        // `x` is emitted via unicode escapes.
        "u0026&num;u007825u0032u0046", // %2F (`u0078` == `x`)
        "u0026&num;u007825u0035u0043", // %5C
        // Hex escape marker `x25` where the leading `x` is emitted via HTML numeric entities.
        "&#120;25u0032u0046", // %2F (`&#120;` == `x`)
        "&#x78;25u0032u0046", // %2F
        "&#120;25u0035u0043", // %5C
        "&#x78;25u0035u0043", // %5C
        ...
        "&#117;0025u0035u0043", // %5C
        "&#x75;0025u0035u0043", // %5C
        // `u0025` where the digits are themselves emitted via hex numeric entities (`&#x30;` == `0`,
        // etc), yielding `u0025` across multiple decode passes.
        "&#x75;&#x30;&#x30;&#x32;&#x35;u0032u0046", // %2F
        "&#x75;&#x30;&#x30;&#x32;&#x35;u0035u0043", // %5C
...
#[test]
fn related_code_query_skips_unicode_emitted_html_entity_path_only_selections() {
    struct PanicSearch<'a> {
        focal_code: &'a str,
    }
    ...
    impl SemanticSearch for PanicSearch<'_> {
        fn search(&self, query: &str) -> Vec<SearchResult> {
            panic!(
                "search should not be called for path-only selections where a unicode escape emits an HTML entity separator; focal_code={}; got query={query}",
                self.focal_code
            );
        }
    }

    // `u0026` decodes to `&`, yielding entities like `&sol;` and `&#47;` across decode passes.
    for focal_code in [
        "u0026sol;homeu0026sol;useru0026sol;secretu0026sol;credentials",
        ...
        assert!(
            req.related_code.is_empty(),
            "expected no related code for unicode-emitted HTML-entity path-only focal code"
        );
    }
}

#[test]
fn related_code_query_skips_hex_emitted_html_entity_path_only_selections() {
    struct PanicSearch<'a> {
        focal_code: &'a str,
    }
    ...
    impl SemanticSearch for PanicSearch<'_> {
        fn search(&self, query: &str) -> Vec<SearchResult> {
            panic!(
                "search should not be called for path-only selections where a hex escape emits an HTML entity separator; focal_code={}; got query={query}",
                self.focal_code
            );
        }
    }
    ...
    for focal_code in [
        ...
        assert!(
            req.related_code.is_empty(),
            "expected no related code for hex-emitted HTML-entity path-only focal code"
        );
    }
}
