...
use nova_fuzzy::{fuzzy_match, TrigramIndexBuilder};
...
fn trigram_build_and_query() {
    ...
    let symbols: Vec<String> = (0..200_000).map(|_| gen_ident(&mut seed)).collect();
    ...
    eprintln!(
        "trigram_query_200k (1000 iters): {:.2?} (total_candidates={total})",
        Instant::now().duration_since(start)
    );
}
