...
"#;
    ...
    assert!(
        diags
            .iter()
            .all(|d| d.code.as_ref() != "invalid-array-access"),
...
#[test]
fn primitive_array_constructor_reference_is_typed_from_target() {
    let src = r#"
interface Maker { int[] make(int n); }
class C {
    void m() {
        Maker x = int[]::new;
    }
...
#[test]
fn primitive_array_constructor_reference_param_mismatch_reports_method_ref_diag() {
    let src = r#"
interface Maker { int[] make(String n); }
class C {
    void m() {
        Maker x = int[]::new;
    }
...
#[test]
fn multidim_array_constructor_reference_is_typed_from_target() {
    let src = r#"
interface Maker { String[][] make(int n); }
class C {
    void m() {
...
#[test]
fn static_context_allows_unqualified_static_field_access_in_static_initializer() {
    let src = r#"
class C {
    static int x;
    static {
...
"#;
    ...
    assert!(
        diags.iter().all(|d| d.code.as_ref() != "static-context"),
        "expected implicit static field access to be allowed in a static initializer; got {diags:?}"
    );
}
...
#[test]
fn type_use_annotation_types_are_ignored_in_local_var_types() {
    let src = r#"
import java.util.List;

class C {
    void m() {
        List<@Missing String> xs = null;
    }
}
"#;
    ...
    assert!(
        !diags
            .iter()
            .any(|d| d.code.as_ref() == "unresolved-type" && d.message.contains("Missing")),
        "expected type-use annotation types in local var declarations to be ignored; got {diags:?}"
    );
}
...
#[test]
fn type_use_annotation_types_are_ignored_in_explicit_generic_invocations() {
    let src = r#"
class C {
    static <T> T id(T t) { return t; }

    void m() {
        C.<@Missing String>id(null);
    }
}
"#;
    ...
    assert!(
        !diags
            .iter()
            .any(|d| d.code.as_ref() == "unresolved-type" && d.message.contains("Missing")),
        "expected type-use annotation types in explicit generic invocations to be ignored; got {diags:?}"
    );
}
...
#[test]
fn lambda_param_type_is_inferred_from_call_argument_target_in_while_condition() {
    let src = r#"
import java.util.function.Function;
class C {
    boolean takes(Function<String, Integer> f) { return true; }
    void m() {
        while (takes(s -> s.length())) {
        }
    }
}
"#;
...
#[test]
fn lambda_param_type_is_inferred_from_call_argument_target_in_for_condition() {
    let src = r#"
import java.util.function.Function;
class C {
    boolean takes(Function<String, Integer> f) { return true; }
    void m() {
        for (; takes(s -> s.length()); ) {
        }
    }
}
"#;
...
"#;
    ...
    assert!(
        diags
            .iter()
            .any(|d| d.code.as_ref() == "lambda-without-target"),
...
#[test]
fn switch_expression_type_is_lub_of_rule_expression_bodies() {
    let src = r#"
class C {
    int m(int n) {
        return switch(n) { case 1 -> 1; default -> 2; };
    }
}
"#;
