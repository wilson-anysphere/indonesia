...
#[test]
fn jpms_typeck_unexported_workspace_package_type_does_not_allow_member_resolution() {
    let mut db = SalsaRootDatabase::default();
    let project = ProjectId::from_raw(0);
    let tmp = TempDir::new().unwrap();
    ...
    let info_a =
        lower_module_info_source_strict("module workspace.a { requires workspace.b; }").unwrap();
    let info_b = lower_module_info_source_strict("module workspace.b { }").unwrap();
...
#[test]
fn jpms_demand_type_of_expr_does_not_resolve_methods_on_unexported_workspace_types() {
    let mut db = SalsaRootDatabase::default();
    let project = ProjectId::from_raw(0);
    let tmp = TempDir::new().unwrap();
...
"#,
    ...
    let call_expr = match root {
        HirStmt::Block { statements, .. } => statements
            ...
            .expect("expected an expression statement for `b.id(\"x\")`"),
        other => panic!("expected a block root statement, got {other:?}"),
    };
