...
fn bazel_workspace_project_model_classifies_overrides_onto_module_path_for_jpms() {
    ...
    // java/com/example).
    let module_root = workspace_root.join("java/com/example");
    ...
    let dep_dir = workspace_root.join("deps/mod-b");
...
fn loads_bazel_targets_as_module_configs() {
    ...
    let model =
        nova_project::load_bazel_workspace_model_with_runner(&workspace_root, &options, runner)
    ...
    let lib_source_root = lib
        ...
        .find(|root| {
            root.origin == SourceRootOrigin::Source
                && root.path.strip_prefix(&workspace_root).unwrap() == Path::new("java/com/example")
        })
    ...
    let lib_generated_root = lib
        ...
        .find(|root| {
            root.origin == SourceRootOrigin::Generated
                && root.path.strip_prefix(&workspace_root).unwrap()
                    == Path::new(
...
fn resolves_aquery_paths_relative_to_execution_root() {
    ...
    assert!(
        lib.source_roots
            .iter()
            .all(|root| root.path.starts_with(&workspace_root)),
    ...
    assert!(
        lib.classpath
            .iter()
            .all(|cp| cp.path.starts_with(&execution_root)),
...
fn reuses_bazel_query_cache_file() {
    ...
    let cache_path = workspace_root.join(".nova/queries/bazel.json");
...
fn loads_bazel_targets_as_workspace_modules() {
    ...
    let lib_file = workspace_root.join("java/com/example/Foo.java");
    assert_eq!(
        model.module_for_path(&lib_file).unwrap().module.id,
        "//java/com/example:lib"
    );

    let generated_file = workspace_root.join(
    ...
    let test_file = workspace_root.join("javatests/com/example/FooTest.java");
    assert_eq!(
