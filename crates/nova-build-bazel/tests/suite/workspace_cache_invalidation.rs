...
#[test]
fn bazelignore_digest_invalidation_triggers_aquery() {
    let dir = tempdir().unwrap();
    let root = dir.path();

    fs::write(root.join("BUILD"), r#"java_library(name = "hello")"#).unwrap();
    fs::write(root.join(".bazelignore"), "ignored_pkg").unwrap();
    ...
    // Editing `.bazelignore` should invalidate the compile-info cache key.
    fs::write(root.join(".bazelignore"), "ignored_pkg\nnewly_ignored").unwrap();
    let _ = workspace.target_compile_info("//:hello").unwrap();
    assert_eq!(runner.count_subcommand("aquery"), 2);
}
...
#[test]
fn bazelrc_import_with_quoted_path_is_tracked() {
    let dir = tempdir().unwrap();
    let root = dir.path();

    fs::write(root.join("BUILD"), r#"java_library(name = "hello")"#).unwrap();
    fs::write(root.join(".bazelrc"), r#"try-import "tools/bazel rc""#).unwrap();
    fs::create_dir_all(root.join("tools")).unwrap();
    fs::write(root.join("tools/bazel rc"), "build --javacopt=-Xlint").unwrap();
...
#[test]
fn bazelrc_transitive_import_digest_invalidation_triggers_aquery() {
    let dir = tempdir().unwrap();
    let root = dir.path();

    fs::write(root.join("BUILD"), r#"java_library(name = "hello")"#).unwrap();
    fs::write(root.join(".bazelrc"), "try-import tools/a.rc").unwrap();
    fs::create_dir_all(root.join("tools")).unwrap();
    fs::write(root.join("tools/a.rc"), "try-import tools/b.rc").unwrap();
    fs::write(root.join("tools/b.rc"), "build --javacopt=-Xlint").unwrap();
    ...
    // Editing a transitively imported `.bazelrc` file should invalidate the cache.
    fs::write(root.join("tools/b.rc"), "build --javacopt=-Xlint:unchecked").unwrap();
    let _ = workspace.target_compile_info("//:hello").unwrap();
    assert_eq!(runner.count_subcommand("aquery"), 2);
}
...
#[test]
#[cfg(unix)]
fn invalidate_changed_files_drops_cache_entries_for_bazelrc_imports_when_changed_path_is_symlink() {
    use std::os::unix::fs::symlink;
    ...
    // Use the canonical (real) workspace root, but invalidate using a symlinked file path.
    let runner = RecordingRunner::default();
    let mut workspace = BazelWorkspace::new(real_root.to_path_buf(), runner.clone()).unwrap();
    ...
    // Invalidate using the symlink path to the imported file; this should still clear the cache
    // entry even though the workspace root is canonical.
    workspace
