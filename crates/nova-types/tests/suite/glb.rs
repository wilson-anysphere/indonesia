...
#[test]
fn infer_upper_bound_intersection_keeps_class_bound_first() {
    let mut env = TypeStore::with_minimal_jdk();
    let object = env.well_known().object;

    let number = env.class_id("java.lang.Number").expect("Number must exist");
    let number = Type::class(number, vec![]);
    let serializable = Type::class(env.well_known().serializable, vec![]);

    // Same bounds, but listed in opposite order. The normalized intersection should keep
    // the class bound (`Number`) first for Java validity/erasure semantics.
    let t1 = env.add_type_param("T1", vec![number.clone(), serializable.clone()]);
    ...
    assert_eq!(
        res1.inferred_type_args,
        vec![Type::Intersection(vec![number, serializable])]
    );
}

#[test]
fn infer_upper_bound_intersection_keeps_class_bound_first_with_named_interface() {
    let mut env = TypeStore::with_minimal_jdk();
    let object = env.well_known().object;
    ...
    let serializable = Type::Named("java.io.Serializable".to_string());

    let t1 = env.add_type_param("T1", vec![z.clone(), serializable.clone()]);
...
#[test]
fn infer_upper_bound_normalizes_single_intersection_bound() {
    let mut env = TypeStore::with_minimal_jdk();
    let object = env.well_known().object;
    ...
    // Equivalent intersections but with different internal ordering.
    let i1 = Type::Intersection(vec![cloneable.clone(), serializable.clone()]);
    let i2 = Type::Intersection(vec![serializable.clone(), cloneable.clone()]);

    let t1 = env.add_type_param("T1", vec![i1]);
