...
#[test]
fn toml_enum_errors_do_not_leak_string_values() {
    use std::error::Error as _;

    // Unknown enum variant errors wrap the variant in backticks.
    let secret = "super-secret-api-key";
    let text = format!(
        r#"
[ai.provider]
...
"#,
    ...
    assert!(
        !message.contains(secret),
        "ConfigError display leaked enum variant value: {message}"
    );
    assert!(
        err.source().is_none(),
        "ConfigError should not expose toml::de::Error as source (may leak raw config text)"
...
#[test]
fn validates_ai_privacy_redact_patterns_do_not_echo_pattern_values() {
    let secret = "super-secret-api-key";
    let text = format!(
        r#"
[ai]
...
"#,
    ...
    match &diagnostics.errors[0] {
        ConfigValidationError::InvalidValue { toml_path, message } => {
            ...
            assert!(
                !message.contains(secret),
                "invalid regex diagnostics should not echo the raw pattern value: {message}"
            );
        }
        other => panic!("expected InvalidValue error, got {other:?}"),
    }
