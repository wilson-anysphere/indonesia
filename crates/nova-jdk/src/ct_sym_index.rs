...
impl CtSymReleaseIndex {
    ...
    pub(crate) fn estimated_bytes(&self) -> u64 {
        ...
        fn class_stub_bytes(stub: &JdkClassStub) -> u64 {
            ...
            bytes = bytes
                .saturating_add((stub.methods.capacity() * size_of::<JdkMethodStub>()) as u64);
        ...
        // Index maps built at construction time.
        bytes =
            bytes.saturating_add((self.class_to_module.capacity() * size_of::<(String, usize)>()) as u64);
        ...
        // Cached stubs (dedup Arc allocations best-effort).
        ...
        if let Some(map) = lock_best_effort(&self.by_internal) {
            bytes =
                bytes.saturating_add((map.capacity() * size_of::<(String, Arc<JdkClassStub>)>()) as u64);
            for (k, v) in map.iter() {
                add_string(&mut bytes, k);
    ...
    pub(crate) fn estimated_bytes(&self) -> u64 {
        ...
        // `module_graph` is omitted here because `ModuleGraph` does not provide a stable way to
        // estimate its heap usage; memory tracking is best-effort.

        bytes = bytes.saturating_add(
            (self.class_to_module.capacity() * size_of::<(String, usize)>()) as u64,
        );
        for (k, _) in self.class_to_module.iter() {
            add_string(&mut bytes, k);
        }
        ...
        for (k, v) in self.internal_to_zip_path.iter() {
            add_string(&mut bytes, k);
            add_string(&mut bytes, v);
        }
