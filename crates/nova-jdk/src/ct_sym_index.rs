...
impl CtSymReleaseIndex {
    ...
    pub(crate) fn estimated_bytes(&self) -> u64 {
        ...
        for module in &self.modules {
            // `ModuleName` is a newtype around `String`.
        ...
        for (internal, _) in &self.class_to_module {
            add_string(&mut bytes, internal);
        }
        ...
        for (internal, zip_path) in &self.internal_to_zip_path {
            add_string(&mut bytes, internal);
            add_string(&mut bytes, zip_path);
        }

        // Cached stubs (avoid double-counting shared `Arc`s between maps).
        ...
        if let Some(stubs) = self.java_lang.get() {
            bytes =
                bytes.saturating_add((stubs.capacity() * size_of::<Arc<JdkClassStub>>()) as u64);
            for stub in stubs {
    ...
    pub(crate) fn estimated_bytes(&self) -> u64 {
        ...
        fn class_stub_bytes(stub: &JdkClassStub) -> u64 {
            ...
            bytes = bytes
                .saturating_add((stub.fields.capacity() * size_of::<crate::stub::JdkFieldStub>()) as u64);
            for field in &stub.fields {
                add_field_stub(&mut bytes, field);
            }
