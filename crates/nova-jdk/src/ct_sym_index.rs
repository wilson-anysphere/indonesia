...
impl CtSymReleaseIndex {
    ...
    pub(crate) fn estimated_bytes(&self) -> u64 {
        ...
        fn class_stub_bytes(stub: &JdkClassStub) -> u64 {
            ...
            bytes =
                bytes.saturating_add((stub.fields.capacity() * size_of::<crate::stub::JdkFieldStub>()) as u64);
            for field in &stub.fields {
                add_field_stub(&mut bytes, field);
            }

            bytes =
                bytes.saturating_add((stub.methods.capacity() * size_of::<crate::stub::JdkMethodStub>()) as u64);
        ...
        if let Some(set) = lock_best_effort(&self.missing) {
            bytes = bytes
                .saturating_add((set.capacity() * size_of::<String>()) as u64);
            for s in set.iter() {
