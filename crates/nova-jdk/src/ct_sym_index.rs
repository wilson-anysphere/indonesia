...
impl CtSymReleaseIndex {
    ...
    pub(crate) fn estimated_bytes(&self) -> u64 {
        ...
        if let Some(map) = lock_best_effort(&self.by_internal) {
            ...
            for (k, stub) in map.iter() {
                add_string(&mut bytes, k);
                bytes = bytes.saturating_add(class_stub_bytes(stub));
            }
        }
    ...
    pub(crate) fn estimated_bytes(&self) -> u64 {
        ...
        // Shallow container sizes.
        bytes = bytes.saturating_add((self.modules.capacity() * size_of::<ModuleName>()) as u64);

        // Key maps (best-effort; ignores HashMap bucket overhead).
        bytes = bytes
            .saturating_add((self.class_to_module.capacity() * size_of::<(String, usize)>()) as u64);
        for (k, _) in &self.class_to_module {
            add_string(&mut bytes, k);
        }
        ...
        // Cached stubs / misses.
        let by_internal = self.by_internal.lock().expect("mutex poisoned");
        bytes =
            bytes.saturating_add((by_internal.capacity() * size_of::<(String, Arc<JdkClassStub>)>()) as u64);
        for (k, _) in by_internal.iter() {
            add_string(&mut bytes, k);
        }
        drop(by_internal);

        let by_binary = self.by_binary.lock().expect("mutex poisoned");
        bytes =
            bytes.saturating_add((by_binary.capacity() * size_of::<(String, Arc<JdkClassStub>)>()) as u64);
        for (k, _) in by_binary.iter() {
            add_string(&mut bytes, k);
        }
        drop(by_binary);

        let missing = self.missing.lock().expect("mutex poisoned");
        ...
        if let Some(pkgs) = self.packages.get() {
            add_vec_string(&mut bytes, pkgs);
        }
        if let Some(names) = self.binary_names_sorted.get() {
            add_vec_string(&mut bytes, names);
        }
        if let Some(java_lang) = self.java_lang.get() {
            bytes = bytes.saturating_add(
                (java_lang.capacity() * size_of::<Arc<JdkClassStub>>()) as u64,
            );
        }
