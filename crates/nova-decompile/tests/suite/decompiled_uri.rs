...
#[test]
fn decompiled_uri_for_classfile_normalizes_internal_name() {
    let uri = decompiled_uri_for_classfile(FOO_CLASS, "com//example\\Foo");
    let expected_hash = content_hash_for(DECOMPILER_SCHEMA_VERSION);
    assert_eq!(
        uri,
        format!("{NOVA_VIRTUAL_URI_SCHEME}:///decompiled/{expected_hash}/com.example.Foo.java")
...
#[test]
fn legacy_uri_helpers_normalize_backslashes() {
    let legacy = uri_for_class_internal_name("com\\example\\Foo");
    assert_eq!(legacy, "nova-decompile:///com/example/Foo.class");
    assert_eq!(
        class_internal_name_from_uri(&legacy).as_deref(),
        Some(FOO_INTERNAL_NAME)
...
#[test]
fn legacy_uri_helpers_reject_traversal() {
    let uri = "nova-decompile:///com/example/../Foo.class";
    assert!(class_internal_name_from_uri(uri).is_none());
}
...
fn legacy_uri_helpers_parse_single_slash_form() {
    let uri = "nova-decompile:/com/example/Foo.class";
    assert_eq!(
        class_internal_name_from_uri(uri).as_deref(),
        Some(FOO_INTERNAL_NAME)
    );
}

#[test]
fn legacy_uri_helpers_reject_missing_class_suffix() {
    let uri = "nova-decompile:///com/example/Foo";
    assert!(class_internal_name_from_uri(uri).is_none());
}
