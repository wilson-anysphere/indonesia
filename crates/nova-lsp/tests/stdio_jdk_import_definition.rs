...
#[test]
fn stdio_definition_into_jdk_resolves_imported_type_name() {
    ...
    let jdk = nova_jdk::JdkIndex::from_jdk_root(&fake_jdk_root).expect("index fake JDK");
    let stub = jdk
    ...
    let offset = text.rfind("List").expect("List token exists");
