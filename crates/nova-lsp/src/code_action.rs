    ...
    pub async fn execute(
        ...
        match action {
            AiCodeAction::ExplainError { diagnostic } => {
                ...
                // Use the Diagnostic's debug format to avoid prompt injection via fence markers
                // in the error message (newlines are escaped and the overall representation is
                // a single, stable line).
                let diagnostic_text = format!("{diagnostic:?}");
...
Edits outside the marked region will be rejected.\n\
...
mod tests {
  ...
  "edits": [
    ...
    #[tokio::test]
    async fn generate_method_body_rejects_patch_outside_insert_range() {
        // This patch is syntactically valid, but it touches line 0 instead of the method body.
        let patch = r#"{
  "edits": [
    {
      "file": "Example.java",
    ...
    #[tokio::test]
    async fn generate_method_body_accepts_patch_within_insert_range() {
        let patch = r#"{
  "edits": [
    {
      "file": "Example.java",
