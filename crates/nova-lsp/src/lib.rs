...
pub(crate) fn sanitize_serde_json_error(err: &serde_json::Error) -> String {
    nova_core::sanitize_json_error_message(&err.to_string())
}
...
pub fn sanitize_error_message(err: &(dyn std::error::Error + 'static)) -> String {
    ...
    sanitize_error_message_text(&message, contains_serde_json)
}
...
mod error_sanitization_tests {
    ...
    fn sanitize_anyhow_error_message_does_not_echo_toml_snippet_blocks() {
        let secret_suffix = "nova-lsp-toml-snippet-secret";
        let message = format!(
    ...
    fn sanitize_anyhow_error_message_does_not_echo_toml_debug_raw_source() {
        let secret_suffix = "nova-lsp-toml-debug-secret";
        let debug = format!(
    ...
    fn sanitize_anyhow_error_message_does_not_echo_toml_single_quoted_values() {
        let secret_suffix = "nova-lsp-toml-single-quote-secret";
...
pub const SEMANTIC_SEARCH_REINDEX_METHOD: &str = "nova/semanticSearch/reindex";
...
mod tests {
    ...
    fn sanitize_serde_json_error_does_not_echo_backticked_values() {
        ...
        let secret_suffix = "nova-lsp-backticked-secret";
