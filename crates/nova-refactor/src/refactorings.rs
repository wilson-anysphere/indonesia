...
fn check_side_effectful_inline_order(
    ...
    // Side-effectful inlining supports more statement containers than normal blocks (notably
    // `switch` groups). Mirror the statement-list indexing logic used in the side-effect safety
    // checks earlier in `inline_variable`.
    let container_stmts: Vec<ast::Statement> =
        if let Some(block) = ast::Block::cast(decl_parent.clone()) {
            block.statements().collect()
        } else if let Some(block) = ast::SwitchBlock::cast(decl_parent.clone()) {
    ...
    let decl_index = container_stmts
        .iter()
    ...
    for target in targets {
        ...
        // Must be within the same statement list as the declaration.
        if usage_stmt.syntax().parent() != Some(decl_parent.clone()) {
            return Err(RefactorError::InlineSideEffects);
        }

        let usage_index = container_stmts
            .iter()
            .position(|stmt| stmt.syntax() == usage_stmt.syntax())
            .ok_or(RefactorError::InlineSideEffects)?;
