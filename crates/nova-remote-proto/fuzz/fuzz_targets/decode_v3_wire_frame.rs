...
fuzz_target!(|data: &[u8]| {
    ...
    runner
        .input_tx
        .send(data.to_vec())
        .expect("decode_v3_wire_frame worker thread exited");

    match runner
        .output_rx
        .lock()
        .expect("decode_v3_wire_frame worker receiver poisoned")
        .recv_timeout(utils::TIMEOUT)
    {
        Ok(()) => {}
        Err(mpsc::RecvTimeoutError::Timeout) => {
            panic!("decode_v3_wire_frame fuzz target timed out")
        }
        Err(mpsc::RecvTimeoutError::Disconnected) => {
            panic!("decode_v3_wire_frame worker thread panicked")
        }
    }
});
