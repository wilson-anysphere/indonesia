...
mod tests {
    ...
    fn workspace_cache_invalidates_when_file_text_changes_in_place_with_same_ptr_and_len() {
        ...
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root = PathBuf::from(format!("/spring-di-cache-inplace-mutation-test-{unique}"));
        let path = root.join("src/main/java/A.java");
        let file_id = FileId::from_raw(0);

        let prefix = "import org.springframework.stereotype.Component;\n@Component class A { /*";
        let suffix = "*/ }\n";
        ...
        let entry1 =
            workspace_entry_with_cancel(&db, file_id, &cancel).expect("expected workspace entry");
        let entry2 =
            workspace_entry_with_cancel(&db, file_id, &cancel).expect("expected cache hit");
        assert!(
            Arc::ptr_eq(&entry1, &entry2),
            "expected spring di workspace entry to be reused from cache"
        );
        ...
        let entry3 = workspace_entry_with_cancel(&db, file_id, &cancel)
            .expect("expected workspace entry rebuild");
        assert!(
            !Arc::ptr_eq(&entry2, &entry3),
            "expected spring di cache to invalidate when file text changes, even when pointer/len are stable"
        );
    }
}
