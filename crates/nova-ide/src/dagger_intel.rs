...
mod tests {
    ...
    fn cache_invalidates_when_file_text_changes_in_place_with_same_ptr_and_len() {
        ...
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root = PathBuf::from(format!("/dagger-cache-inplace-mutation-test-{unique}"));
        let path = root.join("src/main/java/Foo.java");
        let file_id = FileId::from_raw(0);

        // Include a Dagger marker so the analysis runs without build metadata.
        let prefix = "import javax.inject.Inject;\nclass Foo { @Inject /*";
        let suffix = "*/ Object x; }\n";
        ...
        let proj1 =
            project_analysis_with_cancel(&db, db.path.as_path(), &cancel).expect("expected project");
        let proj2 =
            project_analysis_with_cancel(&db, db.path.as_path(), &cancel).expect("expected cache hit");
        assert!(
            Arc::ptr_eq(&proj1, &proj2),
            "expected dagger project to be reused from cache"
        );
        ...
        let proj3 = project_analysis_with_cancel(&db, db.path.as_path(), &cancel)
            .expect("expected rebuild");
        assert!(
            !Arc::ptr_eq(&proj2, &proj3),
            "expected dagger project cache to invalidate when file text changes, even when pointer/len are stable"
        );
    }
}
