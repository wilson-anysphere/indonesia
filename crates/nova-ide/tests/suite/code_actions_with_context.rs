...
#[test]
fn code_actions_with_context_includes_type_mismatch_quickfix_for_cursor_at_span_end() {
    let mut db = InMemoryFileStore::new();
    let file = db.file_id_for_path(PathBuf::from("/test.java"));
    let source = r#"class A {
  void m() {
...
#[test]
fn code_actions_with_context_cast_wraps_conditional_expression_in_parentheses() {
    let mut db = InMemoryFileStore::new();
    let file = db.file_id_for_path(PathBuf::from("/test.java"));
    let source = r#"class A {
  void m() {
...
"#;
    ...
    let cast_fix = cast_fix.expect("expected Cast to byte quickfix");
    assert_eq!(first_edit_new_text(cast_fix), "(byte) (cond?a:b)");
}
...
#[test]
fn code_actions_with_context_includes_return_mismatch_remove_returned_value_quickfix_for_cursor_at_span_end(
) {
    let mut db = InMemoryFileStore::new();
    let file = db.file_id_for_path(PathBuf::from("/test.java"));
    let source = r#"class A {
...
#[test]
fn code_actions_with_context_includes_return_mismatch_quickfix_for_cursor_at_span_end() {
    let mut db = InMemoryFileStore::new();
    let file = db.file_id_for_path(PathBuf::from("/test.java"));
    let source = r#"class A {
  String m() {
