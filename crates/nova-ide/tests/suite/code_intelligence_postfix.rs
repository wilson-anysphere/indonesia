...
#[test]
fn completion_includes_postfix_for_for_fully_qualified_multidim_array_and_replaces_full_expr() {
    let (db, file, pos, text) = fixture(
        r#"
class A {
  void m() {
...
#[test]
fn completion_includes_postfix_for_for_imported_array_and_replaces_full_expr() {
    let (db, file, pos, text) = fixture(
        r#"
...
"#,
    ...
    assert_eq!(edit.range.end, pos);
    assert!(
        edit.new_text.contains("for (List"),
        "expected snippet to contain `for (List`; got {:?}",
        edit.new_text
    );
    assert!(
        edit.new_text.contains(": xs"),
